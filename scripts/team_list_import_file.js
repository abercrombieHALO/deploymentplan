  ...
  reader.onloadend = function(event) {
	...
	if (sheetNames.includes('staff_to_section') && sheetNames.includes('section_to_task')) {
		...
		*/
		...
		if (sheetNames.includes('qualifications')) {
			var quals_sheet = workbook.Sheets['qualifications'];
			
			// convert to html table and insert inside #changes div
			options = { "header": "", "footer": "", "id": "qualificationsTable"};
			var quals_html = XLSX.utils.sheet_to_html(quals_sheet, options);
			
			$("#mySidebar").append(quals_html);
			
			// remove XLSX-generated attributes
			$("#qualificationsTable").find("td")
				.removeAttr('id')
				.removeAttr('t')
				.removeAttr('v');
